...
import { isDispatchHelp, parseFlags, flagShapedPromptTokens } from "./command-args.mjs";
export { parseFlags, flagShapedPromptTokens } from "./command-args.mjs";
import {
  clearPaneComposer,
  escapePaneComposer,
  hasSession,
...
import { cmdCompact } from "./compact.mjs";
...
async function dispatchAgentTarget(name, rest, ctx) {
  ...
  const optionPositionals = separator < 0 ? positional
    : parseFlags(rest.slice(0, separator), FLAG_SPECS.send).positional;
  const strayFlags = flagShapedPromptTokens(optionPositionals);
...
export async function dispatch(argv, ctx) {
  if (isDispatchHelp(argv)) return argv[0] === "compact" ? cmdCompact(ctx, { help: true }) : cmdHelp(ctx);
  const [cmd, ...rest] = argv;
  ...
  switch (cmd) {
    ...
    case "sleep": {
      if (!rest[0]) { console.error("Usage: agent sleep <name|:nr> [-p N] [--force]"); process.exit(1); }
      const name = resolveAgent(rest[0], ctx.configPath);
      const { flags } = parseFlags(rest.slice(1), { p: "number", pane: "number", force: "boolean" });
      const { cmdSleep } = await import("./sleep.mjs");
      return cmdSleep(ctx, name, Number(flags.p || flags.pane || 0), { force: Boolean(flags.force) });
    }

    case "wake": {
      if (!rest[0]) { console.error("Usage: agent wake <name|:nr> [-p N] [--force]"); process.exit(1); }
      const name = resolveAgent(rest[0], ctx.configPath);
      const { flags } = parseFlags(rest.slice(1), { p: "number", pane: "number", force: "boolean" });
      const { cmdWake } = await import("./sleep.mjs");
      return cmdWake(ctx, name, Number(flags.p || flags.pane || 0), { force: Boolean(flags.force) });
    }

    case "sleep-watch": {
      const { flags } = parseFlags(rest, { once: "boolean", dry: "boolean", apply: "boolean" });
      const { cmdSleepWatch } = await import("./sleep.mjs");
      return cmdSleepWatch(ctx, {
    ...
    case "janitor": {
      const { flags, positional } = parseFlags(rest, FLAG_SPECS.janitor);
      if (positional.length) throw new Error("Usage: amux janitor [--dry] [--days N]");
