...
enum class GeneratedLinkComponentTypeId(val wireId: String) { LINK_TARGET_PICKER("link.target-picker"), LINK_TALK("link.talk"), LINK_LATEST_TURN("link.latest-turn"), LINK_COMPOSER("link.composer"), LINK_ACTIVE_PLAYBACK("link.active-playback"), LINK_CONNECTION_STATUS("link.connection-status"), LINK_PUBLIC_LINK("link.public-link"), LINK_PREFERENCES("link.preferences"), LINK_LOCAL_HISTORY("link.local-history"), LINK_UPDATES("link.updates"), LINK_RECOVERY_STATUS("link.recovery-status"), LINK_PAGE_HOST("link.page-host"), LINK_NAVIGATION_ENTRY("link.navigation-entry"), LINK_DEV_PREVIEW("link.dev-preview") }
