...
`AMUX_LINT_BASE_REF` changes contract/style selection, never mandatory size
selection or policy history. Strict changed-size checks always include changes
against locally known trunk plus staged, working and untracked source files,
even when the selected diff base is `HEAD`. Unknown trunk or unreadable/malformed
...
the selected file as a whole, while contracts check only public symbols whose
docs, declaration, or body intersects an added target line.
