...
export function createInboundReconciler({ onMessage, state, seenLimit = DEFAULT_SEEN_LIMIT }) {
  ...
  function enqueue(msg) {
    ...
    const next = previous.catch(() => {}).then(async () => {
      ...
      // `delivered` now means durably accepted by the per-pane FIFO, not that
      // Codex happened to be ready during this callback. A failure here is an
      // enqueue/storage failure: never mark it seen and never give up. The
      // next REST scan retries the same deterministic queue identity.
