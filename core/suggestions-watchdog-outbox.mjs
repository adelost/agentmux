...
import {
  createDeliveryQueue, isNotSentDeliveryJob, waitForDeliveryJob,
...
import {
  HUMAN_DIRECTED_ALERT_KINDS,
  alertTarget,
  assignmentDeliveryPolicy,
  ownerGateMessage,
  ownerNotificationReceipt,
  validateAlerts,
  validateReceipt,
  watchdogAlertDelivery,
} from "./suggestions-watchdog-alerts.mjs";
export {
  HUMAN_DIRECTED_ALERT_KINDS,
  ownerGateMessage,
  ownerNotificationReceipt,
...
export {
  ...
  DEFAULT_ASSIGNMENT_IDLE_MS,
} from "./assignment-delivery-availability.mjs";
...
export function loadWatchdogOutboxConfig(path, { home = homedir(),
  ...
  const baseUrl = normalizeServiceBaseUrl(raw.baseUrl, "config: baseUrl", {
...
export function createAmuxOutboxDeliverer({ queue = createDeliveryQueue(),
  ...
  const deliver = async ({ agent, pane, prompt, idempotencyKey, projectId, alert }) => {
  ...
  deliver.wasAttempted = ({ agent, pane, idempotencyKey }) => {
    const job = queue.list(agent, pane).find((candidate) =>
      candidate.idempotencyKey === idempotencyKey);
    return Boolean(job && !isNotSentDeliveryJob(job)
      && (Number(job.attempts) > 0 || job.status === "acknowledged"
    ...
    try {
      ...
      for (const alert of alerts) {
        try {
          ...
          if (alert.kind === "assignment_offer_delivery" && !alreadyAttempted) {
