...
export function createContextMaintenance({ agent, state, queue, resolveTarget, now = Date.now, log = () => {}, policy = readContextCostPolicy(),
  identityFor = latestPaneSessionIdentity, activityFor = latestConversationActivityMs,
  compactFor = engine => engine === "codex" ? verifiedCodexCompact : verifiedClaudeCompact }) {
  const existing = (name, pane, identity) => {
    const record = state.get(STATE_KEY, {})[paneKey(name, pane)];
    if (!record || (identity && record.sessionId !== identity.sessionId)) return null;
  ...
  async function run(name, pane, { cold = false, leaseHeld = false, jobId = null } = {}) {
    ...
    const activity = activityFor(target.dir, target.engine, { recoverCodexHistory: true });
    ...
    try {
      if (queue.list(name, pane).some(j => j.id !== jobId && !TERMINAL_DELIVERY_STATES.has(j.status) && (!cold || j.status !== "pending"))) {
      ...
      const result = await compact({ agent, agentName: name, pane, paneDir: target.dir,
