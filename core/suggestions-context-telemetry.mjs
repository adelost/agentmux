...
export function nativeContextReading(snapshot, nowMs = Date.now()) {
  ...
  const compactEvent = [...(snapshot.events || [])].reverse().find((event) =>
    event?.type === "web" && ["compacted", "compact-done"].includes(event.subtype));
  return {
    percent: Math.round(raw.percent),
    tokens: Number.isFinite(raw.usedTokens) ? raw.usedTokens : null,
    windowTokens: Number.isFinite(raw.windowTokens) ? raw.windowTokens : null,
    model: snapshot.agent.observedModel || snapshot.agent.model || null,
    effort: snapshot.agent.observedEffort || snapshot.agent.effort || null,
    observedAt: iso(raw.observedAt || snapshot.agent.updatedAt || nowMs),
    source: "native-runtime",
    confidence: "exact",
    lastCompactAt: iso(compactEvent?.at),
  };
}
...
export async function collectContextTelemetry(ctx, {
  ...
  for (const agent of agents) {
    if (!(await hasSession(ctx, agent.name))) continue;
    for (const pane of await listPanes(ctx, agent.name)) {
      const engine = dialectFor(agent, pane);
      if (!engine) continue;
      const { status, preview, context } = await inspectPane(ctx, agent, pane);
      if (!context) continue;
      rows.push({ agent: agent.name, pane: pane.index, engine, status, context, preview,
        label: agent.panes[pane.index]?.label || null });
    }
  }
  return rows;
}
...
export function contextTelemetrySnapshot(rows, generatedAt = new Date().toISOString()) {
  return { version: 1, generatedAt, agents: rows.map(({ agent, pane, engine, status, context }) => ({
    ...
    percent: context.percent, usedTokens: context.tokens ?? null,
    windowTokens: context.windowTokens ?? null, observedAt: context.observedAt ?? null,
    source: context.source ?? `${engine}-unknown`, confidence: context.confidence ?? "estimated",
