...
/** WHAT: Builds one uniform doctor check row. WHY: Keeps every health rule emitting the same report shape. */
export const check = (name, status, detail, hint = "") => ({ name, status, detail, hint });
...
export function checkGuardCronHeartbeats({ heartbeats, now = Date.now() }) {
  ...
  if (disabled.length) {
    return check("guard loops", WARN,
      ...
      "disabled guards are not protection; remove the intentional OFF state when their coverage is required");
  }
